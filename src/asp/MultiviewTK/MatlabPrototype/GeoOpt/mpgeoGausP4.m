...
if nargout > 1
    ...
    if H(1,2) == 0 & H(2,1) == 0 & H(3,1) == 0 & H(3,2) == 0
        ...
        for i=1:n
            Ix(:,:,i) = conv2(gs,ds,Patch.ortho{i},'same');
            Iy(:,:,i) = conv2(ds,gs,Patch.ortho{i},'same');
        end        
        ...
        for i=1:n
            ...
            for k=1:4
                ...
                str=sprintf('zx = ((c22.*b1%d-c12.*b2%d)/H(1,1))./D;',k,k);
                eval(str);
                str=sprintf('zy = ((c11.*b2%d-c21.*b1%d)/H(2,2))./D;',k,k);
                eval(str);
                gdl = gc*reshape(dx(:)'.*zx+dy(:)'.*zy,sz)*gc';
                gdx = gc*reshape(zx'.*Ie(:).^2,sz)*dc';
                gdy = dc*reshape(zy'.*Ie(:).^2,sz)*gc';
                g(i) = g(i)+gdl+gdx+gdy;
            end
        end
    end
